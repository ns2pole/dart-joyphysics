...
result_writer = subprocess.run(
    ["osascript", "writer_electroMagnetism.scpt"],
    capture_output=True,
    text=True
)
...
result = subprocess.run(
    ["osascript", "get_keynote_nodes_electroMagnetism.scpt"],
    capture_output=True,
    text=True
)
...
electroMagnetism_dir = "/Users/nakamurashunsuke/Documents/Youtube/joyphysics/lib/mindMap/electroMagnetism"
# 互換のために electroMagnetism_folder も定義（元コードが使っていた変数名）
electroMagnetism_folder = electroMagnetism_dir

# フォルダ内の最初のファイルを electroMagnetism.png にリネーム（元処理）
if os.path.isdir(electroMagnetism_dir):
    for f in os.listdir(electroMagnetism_dir):
        f_path = os.path.join(electroMagnetism_dir, f)
        if os.path.isfile(f_path):
            new_path = os.path.join(electroMagnetism_dir, "electroMagnetism.png")
            if os.path.exists(new_path):
                os.remove(new_path)
            try:
                os.rename(f_path, new_path)
                print(f"Renamed {f} -> electroMagnetism.png")
            except Exception as e:
                print("ファイルリネームに失敗:", e)
            break
else:
    print("electroMagnetism_dir が存在しません:", electroMagnetism_dir)
    exit(1)

img_path = os.path.join(electroMagnetism_folder, "electroMagnetism.png")
